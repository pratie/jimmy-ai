      ...
      <div className="animate-fade-in opacity-0 delay-300 fill-mode-forwards flex flex-col justify-between h-full pt-6 items-center">
          ...
          {activeAgent ? (
            <>
              ...
              <button
                ...
                className="w-8 h-8 rounded-lg flex items-center justify-center bg-sidebar-muted/40 border border-sidebar-border/30 hover:bg-sidebar-muted text-sidebar-foreground/70 hover:text-sidebar-foreground transition mb-4"
              ...
              <button
                className="w-8 h-8 rounded-lg flex items-center justify-center text-sidebar-foreground/40 hover:text-sidebar-foreground hover:bg-sidebar-muted transition mt-2"
        ...
        <div className="flex flex-col items-center gap-3">
          {activeAgent && (
            <button
              onClick={handleBackToAgents}
              className="w-8 h-8 rounded-lg flex items-center justify-center text-sidebar-foreground/60 hover:text-rose-600 hover:bg-rose-50 transition-all"
              title="Exit Preview"
            >
          ...
          {user && (
            <div className="relative border-t border-sidebar-border/40 pt-3">
              <button
                ...
                className="flex items-center justify-center w-8 h-8 rounded-full bg-sidebar-muted border border-sidebar-border hover:bg-sidebar-muted/80 transition text-xs font-bold"
                title={user.fullname}
              >
                {user.fullname.charAt(0).toUpperCase()}
              </button>
              {showDropdown && (
                <div className="absolute bottom-full left-full ml-2 mb-0 bg-sidebar/95 backdrop-blur border border-sidebar-border rounded-lg shadow-large overflow-hidden w-48 z-50">
                  <div className="px-4 py-3 border-b border-sidebar-border bg-sidebar-muted/20">
                    ...
                    <p className="text-[10px] text-sidebar-foreground/60 truncate mt-0.5">{user.email}</p>
                  </div>
                  <button
                    onClick={() => {
                    ...
                    className="flex items-center gap-2 px-4 py-2.5 w-full hover:bg-sidebar-muted transition-colors text-left font-semibold text-sidebar-foreground/90"
                  >
